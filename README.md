...
analyzer, invoice import, and point-of-sale checkout.
...
- `storage.py` — persists any named table (deals, inventory, ...) per user
  via the same Apps Script backend
...
- `stripe_webhooks.py` — pure, unit-tested Stripe webhook signature
  verification + event handling; not yet wired to a live endpoint (see
  DEPLOY.md)
...
- `mail.py` / `mail_parse.py` — optional read-only Gmail tracking of
  supplier invoices/shipments; parsing logic is unit-tested, not yet wired
  to a UI tab
