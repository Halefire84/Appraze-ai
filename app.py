...
# anyone could hand-craft that URL) before recording the plan.
# --------------------------------------------------------------------------
_qp = st.query_params
if _qp.get("sub_session_id") and _qp.get("sub_plan"):
    _sub_plan_key = _qp["sub_plan"]
    _sub_result = verify_checkout_session(_qp["sub_session_id"])
    if _sub_result.paid:
        if mark_paid(st.session_state.get("username", ""), _sub_plan_key):
