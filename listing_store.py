...
Listings are stored separately from flip inventory so one master item can have
multiple marketplace drafts. Publishing remains an integration concern.
"""
...
def transition_listing(listing: Dict[str, Any], status: str, *, external_id: str = None) -> Dict[str, Any]:
    ...
    current = str(listing.get("status", "DRAFT")).upper()
    next_status = str(status).upper()
    if current not in LISTING_STATUSES:
        raise ValueError(f"Unsupported current listing status: {current}")
    if next_status not in LISTING_STATUSES:
        raise ValueError(f"Unsupported listing status: {next_status}")
    if next_status not in ALLOWED_LISTING_TRANSITIONS[current]:
        raise ValueError(f"Invalid listing transition: {current} -> {next_status}")
    item = dict(listing)
...
def mark_item_sold(listings: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Mark every non-terminal marketplace listing SOLD after a sale."""
    result = []
    for row in listings:
        item = dict(row)
