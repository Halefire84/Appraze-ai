...
from deal_settings import (
    DealMathSettings,
    category_fee_pct as _resolve_category_fee_pct,
    estimate_shipping_cost as _estimate_shipping_cost,
)
...
def _resolve_deal_costs(resale_value, fee_pct, category, settings, weight_lb, packaging_type, shipping_cost, insurance_cost, ad_pct):
    """Shared setup for calc_deal / max_cost_for_target_roi: resolve the
    category-aware fee (brief item 1) and weight-tier shipping + packaging
    (brief item 2) when the caller didn't pin down an explicit number,
    without changing behavior for callers that never pass a category
    (existing flat fee_pct/premium_pct defaults are untouched).

    ad_pct (promoted-listing spend) is deliberately a separate knob from
    fee_pct -- per the brief, it must NEVER be folded into the base resale
    fee, so it's applied as its own line against resale_value.
    """
    settings = settings or DealMathSettings()
    if fee_pct is None:
        fee_pct = _resolve_category_fee_pct(category, settings) if category else DEFAULT_FEE_PCT

    ship = shipping_cost
    if ship is None and (weight_lb is not None or category is not None):
        ship, _tier_cost, _pack_cost, _source = _estimate_shipping_cost(weight_lb, category, packaging_type, settings)
    ship = ship or 0.0
    ins = insurance_cost or 0.0
    if ad_pct:
        try:
            ad_amount = float(resale_value) * (ad_pct / 100.0)
        except (TypeError, ValueError):
            ad_amount = 0.0
    else:
        ad_amount = 0.0
    return fee_pct, ship, ins, ad_amount
...
def calc_deal(
    ...
    *,
    ...
    fee_pct:       platform resale fee, e.g. eBay/Mercari ~13%. If omitted,
                   resolved from `category` via deal_settings' category fee
                   table (brief item 1) -- falls back to DEFAULT_FEE_PCT
                   when no category is given either, so existing callers are
                   unaffected.
    premium_pct:   buyer's premium at purchase, e.g. CTBids 18%
    category / weight_lb / packaging_type: when shipping_cost is omitted,
                   used to estimate outbound resale shipping + packaging
                   from the weight-tier table (brief item 2) instead of
                   silently assuming $0.
    shipping_cost / insurance_cost: explicit $ amounts, deducted from the
                   resale side alongside the platform fee.
    ad_pct:        optional promoted-listing ad spend, percentage of
                   resale_value. Kept entirely separate from fee_pct -- the
                   brief is explicit that ad spend must never be folded into
                   the base resale fee.
    """
    resolved_fee_pct, ship, ins, ad_amount = _resolve_deal_costs(
        resale_value, fee_pct, category, settings, weight_lb, packaging_type, shipping_cost, insurance_cost, ad_pct,
    )

    if _finance_invalid(cost, resale_value, resolved_fee_pct, premium_pct or DEFAULT_PREMIUM_PCT, ship, ins):
    ...
    premium_pct = DEFAULT_PREMIUM_PCT if premium_pct is None else premium_pct
    true_cost = cost * (1 + premium_pct / 100)
    net_resale = resale_value * (1 - resolved_fee_pct / 100) - ship - ins - ad_amount
    gross_profit = net_resale - true_cost
...
def max_cost_for_target_roi(
    ...
    *,
    ...
    produces exactly the target ROI. Same category-aware fee_pct / weight-tier
    shipping resolution as calc_deal (brief items 1-2); see its docstring.
    """
    resolved_fee_pct, ship, ins, ad_amount = _resolve_deal_costs(
        resale_value, fee_pct, category, settings, weight_lb, packaging_type, shipping_cost, insurance_cost, ad_pct,
    )
