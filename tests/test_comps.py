...
class TestSummarizeComps(unittest.TestCase):
    ...
    def test_single_sold_comp(self):
        # A single comp is thin evidence (< thin_comp_count_threshold), so the
        # raw median gets a thin-comp haircut applied on top of the base value.
        summary = summarize_comps([sold(100)])
        self.assertEqual(summary.count, 1)
        self.assertEqual(summary.sold_count, 1)
        self.assertEqual(summary.raw_suggested_value, 100)
        self.assertTrue(summary.thin_comps)
        self.assertEqual(summary.suggested_value, 100 * (1 - summary.haircut_pct / 100.0))
        self.assertEqual(summary.confidence, CONFIDENCE_LOW)

    def test_median_of_sold_comps_used_as_suggested_value(self):
        # active(500) is >2.5x the ~100-110 median of all 4 comps, so outlier
        # trimming drops it entirely before sold/active counts are taken.
        ...
        self.assertEqual(summary.trimmed_outlier_count, 1)
        self.assertFalse(summary.thin_comps)  # 3 surviving comps clears the thin-comp bar

    def test_falls_back_to_all_comps_when_no_sold(self):
        # Only 2 comps survive (no outliers to trim here), which is below the
        # thin-comp threshold, so a haircut applies to the raw median.
        comps = [active(90), active(110)]
        summary = summarize_comps(comps)
        self.assertEqual(summary.raw_suggested_value, 100)  # median of active, since no sold
        self.assertTrue(summary.thin_comps)
        self.assertEqual(summary.suggested_value, 100 * (1 - summary.haircut_pct / 100.0))
        self.assertEqual(summary.confidence, CONFIDENCE_LOW)
